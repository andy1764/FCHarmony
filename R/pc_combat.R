#' Harmonization using principal components of matrix
#'
#' @param x
#' @param bat
#' @param mod
...
#'
#' @examples
pc_combat <- function(x, # array of fc matrices, roi x roi x nsubj
  ...
  if (pc.sym) {
    v_vec <- t(apply(x, 3, function(x) c(x[lower.tri(x, diag = TRUE)])))
  } else {
    v_vec <- t(apply(x, 3, c))
  }
